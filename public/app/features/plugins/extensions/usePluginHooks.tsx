...
import { PluginExtensionHook, PluginExtensionTypes } from '@grafana/data/src/types/pluginExtensions';
import {
...
export function usePluginHooks<Signature>({
  ...
  return useMemo(() => {
    ...
    const results: Array<PluginExtensionHook<Signature>> = [];
    ...
    for (const registryItem of registryState?.[extensionPointId] ?? []) {
      ...
      results.push({
        ...
        type: PluginExtensionTypes.hook,
        title: registryItem.title,
        description: registryItem.description ?? '',
        pluginId: pluginId,
        hook: registryItem.hook,
      });
